...
static int __init ls2h_rtc_probe(struct platform_device *pdev)
{
...
static int __exit ls2h_rtc_remove(struct platform_device *pdev)
{
...
static struct platform_driver ls2h_rtc_driver = {
	...
	.remove = __exit_p(ls2h_rtc_remove),
	.driver = {
		   .name = "ls2h-rtc",
		   .owner = THIS_MODULE,
		   },
