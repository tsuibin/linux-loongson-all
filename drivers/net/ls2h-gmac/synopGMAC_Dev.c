...
/**
  ...
  */
s32 synopGMAC_read_phy_reg(u32 * RegBase, u32 PhyBase, u32 RegOffset,
			   u16 * data)
{
	u32 addr;
	u32 loop_variable;
	...
	if (loop_variable < DEFAULT_LOOP_VARIABLE)
		*data =
		    (u16) (synopGMACReadReg(RegBase, GmacGmiiData) & 0xFFFF);
	else {
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacConfig,
			   GmacFrameBurst);
	return;
}
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacConfig,
			 GmacDeferralCheck);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacFrameFilter,
			   GmacFilter);
	return;
}
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacFrameFilter,
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacFlowControl,
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacFlowControl,
			   GmacUnicastPauseFrame);
	return;
}
...
void synopGMAC_tx_activate_flow_control(synopGMACdevice * gmacdev)
{
	...
	if (gmacdev->Speed == FULLDUPLEX) {
		if (!synopGMACCheckBits
		    ((u32 *) gmacdev->MacBase, GmacFlowControl,
		     GmacFlowControlBackPressure))
			synopGMACSetBits((u32 *) gmacdev->MacBase,
					 GmacFlowControl,
					 GmacFlowControlBackPressure);
...
void synopGMAC_pause_control(synopGMACdevice * gmacdev)
{
	...
	synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacFlowControl,
			  mac_flow_control_reg);
...
s32 synopGMAC_mac_init(synopGMACdevice * gmacdev)
{
	if (gmacdev->DuplexMode == FULLDUPLEX) {
		...
		synopGMACSetBits((u32 *) gmacdev->DmaBase, DmaControl,
				 DmaFwdErrorFrames);
		synopGMACSetBits((u32 *) gmacdev->DmaBase, DmaControl,
				 0x02000000);
		...
		if (gmacdev->Speed == SPEED1000)
			...
			if (gmacdev->Speed == SPEED100)
				synopGMACSetBits(gmacdev->MacBase,
						 GmacConfig, GmacFESpeed100);
			else
				synopGMACSetBits(gmacdev->MacBase,
						 GmacConfig, GmacFESpeed10);
		}
		...
		synopGMACSetBits(gmacdev->DmaBase, DmaControl,
				 DmaStoreAndForward);
		synopGMACSetBits(gmacdev->DmaBase, DmaControl,
				 DmaFwdErrorFrames);
		synopGMAC_tx_enable(gmacdev);
...
/**
  ...
  */
s32 synopGMAC_set_mac_addr(synopGMACdevice * gmacdev, u32 MacHigh, u32 MacLow,
			   u8 * MacAddr)
{
	u32 data;
	...
	data =
	    (MacAddr[3] << 24) | (MacAddr[2] << 16) | (MacAddr[1] << 8) |
	    MacAddr[0];
...
/**
  ...
  * \return 0 upon success. Error code upon failure.
  */
s32 synopGMAC_get_mac_addr(synopGMACdevice * gmacdev, u32 MacHigh, u32 MacLow,
			   u8 * MacAddr)
{
	u32 data;
...
s32 synopGMAC_attach(synopGMACdevice * gmacdev, void *macBase, void *dmaBase,
		     u32 phyBase, u8 * mac_addr0)
{
...
{
	synopGMACWriteReg((u32 *) gmacdev->DmaBase, DmaRxBaseAddr,
...
{
	return (((status & DescTxLateCollision) ==
		 DescTxLateCollision) | ((status & DescTxExcCollisions) ==
					 DescTxExcCollisions));
...
{
	return (((status & DescTxLostCarrier) ==
		 DescTxLostCarrier) | ((status & DescTxNoCarrier) ==
				       DescTxNoCarrier));
}
...
//bool synopGMAC_dma_rx_collisions(u32 status)
	return (((status & DescRxDamaged) ==
		 DescRxDamaged) | ((status & DescRxCollision) ==
				   DescRxCollision));
}
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_get_desc_data(DmaDesc * desc, u32 * Status, dma_addr_t * Buffer1,
			     u32 * Length1, u64 * Data1, dma_addr_t * Buffer2,
			     u32 * Length2, u64 * Data2)
{
...
s32 synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev, u32 * Status,
			  u32 * Buffer1, u32 * Length1, u64 * Data1,
			  u32 * Buffer2, u32 * Length2, u64 * Data2,
			  ...
			  u32 * Time_Stamp_Low)
{
	...
	TR("%02d %08x %08x %08x %08x %08x %08x %08x\n", txover, (u32) txdesc,
	   txdesc->status, txdesc->length, txdesc->buffer1, txdesc->buffer2,
			  ...
			  u32 * Buffer2, u32 * Length2, u64 * Data2)
{
	...
	TR("txover:%02d txdesc:%p status:%08x length:%08x buf1:%08x buf2:%08x data1:%lx data2:%lx\n", txover, txdesc,
	   txdesc->status, txdesc->length, txdesc->buffer1, txdesc->buffer2,
	   txdesc->data1, txdesc->data2);
...
/**
  ...
  * \return returns present tx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_set_tx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1,
			  u64 Data1, u32 Buffer2, u32 Length2, u64 Data2,
			  u32 offload_needed)
{
	//spin_lock_irq(&gmacdev->lock);
...
//              synopGMAC_tx_checksum_offload_tcp_pseudo(gmacdev, txdesc);
		...
		gmacdev->TxNext =
		    synopGMAC_is_last_tx_desc(gmacdev, txdesc) ? 0 : txnext + 1;
...
//              synopGMAC_tx_checksum_offload_tcp_pseudo(gmacdev, txdesc);
	...
	TR("txnext:%02d txdesc:%p status:%08x length:%08x buf1:%08x buf2:%08x data1:%lx data2:%lx\n", txnext, txdesc,
	   txdesc->status, txdesc->length, txdesc->buffer1, txdesc->buffer2,
	   txdesc->data1, txdesc->data2);
	return txnext;
}
...
/**
  ...
  */
s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1,
			  u64 Data1, u32 Buffer2, u32 Length2, u64 Data2)
{
	//spin_lock_irq(&gmacdev->lock);
	...
	if (synopGMAC_is_rx_desc_chained(rxdesc)) {
		...
		gmacdev->RxNext =
		    synopGMAC_is_last_rx_desc(gmacdev, rxdesc) ? 0 : rxnext + 1;
	...
	TR("%02d %p %08x %08x %08x %08x %lx %lx\n", rxnext, rxdesc,
	   rxdesc->status, rxdesc->length, rxdesc->buffer1, rxdesc->buffer2,
...
{
	synopGMACWriteReg((u32 *) gmacdev->DmaBase, DmaInterrupt,
			  DmaIntDisable);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacInterruptMask,
			   GmacPmtIntMask);
	return;
}
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacInterruptMask,
			 GmacPmtIntMask);
	return;
}
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacPmtCtrlStatus,
			 GmacPmtPowerDown);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacPmtCtrlStatus,
			   GmacPmtPowerDown);
	return;
}
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_write_wakeup_frame_register(synopGMACdevice * gmacdev,
					   u32 * filter_contents)
{
	s32 i;
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacPmtCtrlStatus,
	...
	for (i = 0; i < WAKEUP_REG_LENGTH; i++)
		synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacWakeupAddr,
				  *(filter_contents + i));
	return;
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacMmcCntrl,
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacMmcCntrl,
			   GmacMmcCounterFreeze);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacMmcRxIpcIntrMask,
			   mask);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->DmaBase, DmaControl,
			   DmaDisableDropTcpCs);
	return;
}
...
/**
  ...
  */
bool synopGMAC_is_tx_ipv4header_checksum_error(synopGMACdevice * gmacdev,
					       u32 status)
{
...
/**
  ...
  * \return returns true if error in ipv4 header checksum, else returns false.
  */
bool synopGMAC_is_tx_payload_checksum_error(synopGMACdevice * gmacdev,
					    u32 status)
{
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_tx_checksum_offload_bypass(synopGMACdevice * gmacdev,
					  DmaDesc * desc)
{
#ifdef ENH_DESC
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_tx_checksum_offload_ipv4hdr(synopGMACdevice * gmacdev,
					   DmaDesc * desc)
{
#ifdef ENH_DESC
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_tx_checksum_offload_tcponly(synopGMACdevice * gmacdev,
					   DmaDesc * desc)
{
#ifdef ENH_DESC
...
/**
  ...
  * \return returns void.
  */
void synopGMAC_tx_checksum_offload_tcp_pseudo(synopGMACdevice * gmacdev,
					      DmaDesc * desc)
{
#ifdef ENH_DESC
	desc->status =
	    ((desc->length & (~DescTxCisMask)) | DescTxCisTcpPseudoCs);
#else
	desc->length =
	    ((desc->length & (~DescTxCisMask)) | DescTxCisTcpPseudoCs);
#endif
...
{
	synopGMACSetBits((u32 *) gmacdev->MacBase, GmacTSControl,
			 GmacTSENMACADDR);
	return;
}
...
{
	synopGMACClearBits((u32 *) gmacdev->MacBase, GmacTSControl,
			   GmacTSENMACADDR);
	return;
}
...
/**
  ...
  * \return returns void
  */
void synopGMAC_TS_subsecond_init(synopGMACdevice * gmacdev,
				 u32 sub_sec_inc_value)
{
	synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacTSSubSecIncr,
...
/**
  ...
  */
void synopGMAC_TS_read_timestamp(synopGMACdevice * gmacdev,
				 u16 * higher_sec_val, u32 * sec_val,
				 u32 * sub_sec_val)
{
	*higher_sec_val =
	    (u16) (synopGMACReadReg((u32 *) gmacdev->MacBase, GmacTSHighWord) &
		   GmacTSHighWordMask);
...
/**
  ...
  * \return returns void
  */
void synopGMAC_TS_load_timestamp_higher_val(synopGMACdevice * gmacdev,
					    u32 higher_sec_val)
{
	synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacTSHighWord,
			  (higher_sec_val & GmacTSHighWordMask));
	return;
}
...
/**
  ...
  * \return returns void
  */
void synopGMAC_TS_read_timestamp_higher_val(synopGMACdevice * gmacdev,
					    u16 * higher_sec_val)
{
	*higher_sec_val =
	    (u16) (synopGMACReadReg((u32 *) gmacdev->MacBase, GmacTSHighWord) &
...
/**
  ...
  * \return returns 0 for Success or else Failure
  */
void synopGMAC_TS_load_target_timestamp(synopGMACdevice * gmacdev, u32 sec_val,
					u32 sub_sec_val)
{
	synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacTSTargetTimeHigh,
			  sec_val);
	synopGMACWriteReg((u32 *) gmacdev->MacBase, GmacTSTargetTimeLow,
			  sub_sec_val);
	return;
}
...
{
	*sec_val =
	    synopGMACReadReg((u32 *) gmacdev->MacBase, GmacTSTargetTimeHigh);
	*sub_sec_val =
	    synopGMACReadReg((u32 *) gmacdev->MacBase, GmacTSTargetTimeLow);
	return;
}
