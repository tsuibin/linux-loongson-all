...
static int probe(struct pci_dev *pdev, const struct pci_device_id *my_pci_id)
{
	 ...
	 */
	synopGMACMappedAddr =
	    (u8 *) ioremap_nocache((u32) the_register_resource_base,
				   (size_t) (128 * 1024));
	...
	if (check_mem_region
	    ((resource_size_t) synopGMACMappedAddr, synopGMACMappedAddrSize)) {
	...
	request_mem_region((resource_size_t) synopGMACMappedAddr,
			   synopGMACMappedAddrSize, "synopGMACmemory");
	...
	if ((synopGMACadapter =
	     synopGMAC_init_network_interface(&pdev->dev, synopGMACMappedAddr,
...
static struct pci_driver pci_driver = {.name = synopGMAC_driver_name,
	.id_table = ids,
	.probe = probe,
	.remove = remove,
};
